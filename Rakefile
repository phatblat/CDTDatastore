...
task :testencryptionios do
    # build using xcpretty as otherwise it's very verbose when running tests
  $ios_success = system("xcodebuild -workspace EncryptionTests/EncryptionTests.xcworkspace -scheme 'Encryption Tests' -destination 'platform=iOS Simulator,OS=8.1,name=iPhone 4S' build | xcpretty; exit ${PIPESTATUS[0]}")
  unless $ios_success
    puts "** Build failed"
    exit(-1)
  end
  $ios_success = system("xcodebuild -workspace EncryptionTests/EncryptionTests.xcworkspace -scheme 'Encryption Tests' -destination 'platform=iOS Simulator,OS=8.1,name=iPhone 4S' test")
  puts "\033[0;31m! iOS unit tests failed with status code #{$?}" unless $ios_success
  if $ios_success
    puts "** All tests executed successfully"
  else
