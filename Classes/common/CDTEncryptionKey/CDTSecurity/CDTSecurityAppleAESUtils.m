...
@interface CDTSecurityAppleAESUtils ()

@end

@implementation CDTSecurityAppleAESUtils

#pragma mark - CDTSecurityAESUtils methods
- (NSData *)doEncrypt:(NSData *)text key:(NSString *)key withIV:(NSString *)iv
{
    return [CDTSecurityAppleAESUtils applyOperation:kCCEncrypt toData:text withKey:key iv:iv];
}

- (NSData *)doDecrypt:(NSData *)text key:(NSString *)key withIV:(NSString *)iv
{
    return [CDTSecurityAppleAESUtils applyOperation:kCCDecrypt toData:text withKey:key iv:iv];
}
                        ...
                        iv:(NSString *)iv
{
    ...
    NSAssert(cryptorStatus == kCCSuccess, @"Data not encrypted (1)");
    ...
    NSAssert(cryptorStatus == kCCSuccess, @"Data not encrypted (2)");
