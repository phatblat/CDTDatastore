...
- (BOOL)updateIndex:(CDTIndex *)index
{
    ...
    CDTFetchChanges *fetcher = [[CDTFetchChanges alloc] initWithDatastore:_datastore
                                              previousServerSequenceValue:lastSeqString];
    
    CDTIndexManager *weakSelf = self;
    ...
    // We need this to be syncronous, so it's complete before the
    // query starts. Use a latch in the completion block to make
    // sure this happens.
    ...
    [fetcher start];  // sync right now
    
    // Wait 5 minutes
    dispatch_semaphore_wait(latch, dispatch_walltime(DISPATCH_TIME_NOW, 300 * NSEC_PER_SEC));
